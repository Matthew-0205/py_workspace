...
def hold_reg_to_float64(register0, register1, register2, register3) -> float:
    return struct.unpack(">d", struct.pack('>HHHH', register0, register1, register2, register3))[0]
...
def read_float(client: ModbusSerialClient, address):
    result = client.read_holding_registers(address = address, count = 2,slave = 1)
    if not result.isError():
        return hold_reg_to_float32(result.registers[0], result.registers[1])
    else: 
        print(f"Error reading float at address {address}")

def read_int16(client: ModbusSerialClient, address):
    result = client.read_holding_registers(address = address, count = 2,slave = 3)
    if not result.isError():
        return hold_reg_to_int16(result.registers[0])
    else: 
        print(f"Error reading int16 at address {address}")
    
def read_uint16(client: ModbusSerialClient, address):
    result = client.read_holding_registers(address = address, count = 2,slave = 3)
    if not result.isError():
...
def read_int32(client: ModbusSerialClient, address):
    result = client.read_holding_registers(address = address, count = 2,slave = 3)
    if not result.isError():
...
def read_uint32(client: ModbusSerialClient, address):
    result = client.read_holding_registers(address = address, count = 2,slave = 1)
    if not result.isError():
...
def read_string(client: ModbusSerialClient, address, len):
    result = client.read_holding_registers(address = address, count = len,slave = 1)
    if not result.isError():
        return hold_reg_to_string(result.registers,len)
    else: 
